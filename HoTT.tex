...
\frame
  {
    ...
    \begin{itemize}[<+->]
    \item We can infer the following jugment:
...
\frame
  {
    ...
    \item Proof: by induction, using $isequiv(id_A)$
 ...
 \frame
  {
    ...
    \item Propositional equalities (use functional extensionality):
      \begin{itemize}[<+->]
      \item $happly\,(funext\,h) = h$
      \item $\alpha = funext\,(happly\,\alpha)$
      \item $refl_f = funext\,(\lamt{x}{A}{refl_{fx}})$
 ...
 \frame
  {
    ...
    \item LEM: under conditions as above:
...
\frame
  {
    ...
    \begin{itemize}[<+->]
    \item $idtoeqv: (A=_\UU B) \to (A\simeq B)$ defined by path induction
    \item Univalence Axiom: for all $A,B:\UU$, $isequiv(idtoeqv)$; hence:
          $$(A=_\UU B) \simeq (A\simeq B)$$
